...
def convert_qiskit_to_qulacs(qc):
    qasm_str = qasm2.dumps(qc)
    qasm_list = [replace_angle_with_value(x) for x in qasm_str.split("\n")]
    return convert_QASM_to_qulacs_circuit(qasm_list)


def replace_angle_with_value(text):
    # Define the regex pattern to match "pi/number"
    pattern = r'\((.*?)\)'

    # Function to convert the matched pattern to a floating-point value
    def convert_to_value(match):
        # Extract the matched string
        matched_string = match.group(1)
        # Replace 'pi' with math.pi and evaluate the expression
        value = eval(matched_string.replace('pi', 'math.pi'))
        # Return the value as a string
        value = f"({value})"
        return value

    # Use re.sub with a function to replace all occurrences of the pattern
    result = re.sub(pattern, convert_to_value, text)
    return result
